...
def donothing():
    ...
    filewin.geometry('720x280')
    lbl = tk.Label(filewin, text="This is a Project by Gaurav Ratnakar and Abhishek Shujalpurkar"
                   , fg='red', font=("Helvetica", 16))
    lbl.place(x=60, y=50)
    btn = tk.Button(filewin, text="Close", fg='blue', )
...
message = tk.Label(window, text="Face-Recognition-Based-Attendance-Management-System", bg="Green", fg="white", width=50,
                   height=3, font=('times', 30, 'bold underline'))
...
def TrackImages(msql=None):
    ...
    # adding csv file to the db

    data = pd.read_csv(fileName)
    AttendanceData = pd.DataFrame(data, columns=['Id', 'Name', 'Date', 'Time'])
    AttendanceData.head()
    try:
        conn = mysql.connector.connect(host="localhost", database="AttendanceDB", user="root", password="root")
        if conn.is_connected():
            cursor = conn.cursor()
            cursor.execute("CREATE TABLE Attendance(Id varchar(255),NAME varchar(255),DATEOFARRIVAL DATE,TIME varchar(255))")
            #STR_TO_DATE( '17/07/2013 18:33:55', '%d/%m/%Y %H:%i:%s')
            for i, row in AttendanceData.iterrows():
                sql = """INSERT INTO AttendanceDB.Attendance VALUES(%s,%s,%s,%s)"""
